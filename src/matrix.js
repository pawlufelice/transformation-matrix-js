/*!
	2D Transformation Matrix v2.0, (rewritten for ES6 by Tracdelight)
 ...
 */
 export default class Matrix{
	...
	constructor() {
		...
		if(arguments) {
			['a', 'b', 'c', 'd', 'e', 'f'].forEach((attr, i) => {
				if(typeof (arguments[i]) !== 'undefined') this[attr] = arguments[i];
			});
		}
	}
	 ...
	 */
	concat(cm) {
		return this.clone().transform(cm.a, cm.b, cm.c, cm.d, cm.e, cm.f);
	}
	...
	reflectVector({x, y}) {
		...
		var v = this.applyToPoint({x: 0, y: 1}),
			d = 2 * (v.x * x + v.y * y);
	...
	applyToArray(points) {
		...
		while( p = points[i++]) {
			mxPoints.push(this.applyToPoint(p));
		}
	...
	applyToTypedArray(points, use64) {
		...
		while(i < l) {
			const {x, y} = this.applyToPoint({x: points[i], y: points[i + 1]});
			mxPoints[i++] = x;
			mxPoints[i++] = y;
		}
